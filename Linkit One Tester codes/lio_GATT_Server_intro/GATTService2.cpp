...
static LGATTServiceInfo g_serv1_decl[] =
{
  { TYPE_SERVICE, "E30A39F4-73F5-4BC4-A12F-17D1AD07A962", TRUE, 0, 0, 0 },
    ...
    { TYPE_CHARACTERISTIC, "01234567-89AB-CDEF-0123-456789ABCDF1", FALSE, VM_GATT_CHAR_PROP_NOTIFY|VM_GATT_CHAR_PROP_READ, VM_GATT_PERM_READ, 0 },
    { TYPE_CHARACTERISTIC, "01234567-89AB-CDEF-0123-456789ABCDF2", FALSE, VM_GATT_CHAR_PROP_WRITE, VM_GATT_PERM_WRITE, 0 },
  { TYPE_END, 0, 0, 0, 0, 0 }
};

// prepare the data for profile
LGATTServiceInfo *GATTService2::onLoadService(int32_t index) {
  Serial.printf( "[GATTService2]  onLoadService()\n" );
...
boolean GATTService2::onCharacteristicAdded(LGATTAttributeData &data) {
    Serial.printf( "[GATTService2]  onCharacteristicAdded()\n" );
...
boolean GATTService2::onConnection( const LGATTAddress &addr, boolean connected ) {
  Serial.printf( "[GATTService2] Client %X:%X:%X:%X:%X:%x ",
...
boolean GATTService2::onRead( LGATTReadRequest &data ) {
    Serial.println( "[GATTService2] onRead()" );
    if( _connected ) {
...
boolean GATTService2::onWrite( LGATTWriteRequest &data ) {
  Serial.println( "[GATTService2]  onWrite()" );
